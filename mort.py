...
# Function to calculate total interest paid for fixed mortgage
def calculate_total_interest_fixed(principal, annual_rate, years):
    ...
    total_paid = monthly_payment * int(years * 12)
    total_interest = total_paid - principal
    return total_interest

# Function to calculate total interest paid for variable mortgage
def calculate_total_interest_variable(principal, start_rate, rate_drop, fixed_rate, drop_months, fixed_months):
    ...
    for i in range(int(drop_months)):
        monthly_rate = (start_rate - i * rate_drop) / 12 / 100
        interest = balance * monthly_rate
        total_interest += interest
        principal_payment = calculate_fixed_mortgage_payment(balance, start_rate - i * rate_drop, (drop_months - i) / 12) - interest
        balance -= principal_payment
    # Remaining months with fixed rate
    for i in range(int(fixed_months)):
        monthly_rate = fixed_rate / 12 / 100
        interest = balance * monthly_rate
        total_interest += interest
        principal_payment = calculate_fixed_mortgage_payment(balance, fixed_rate, (fixed_months - i) / 12) - interest
        balance -= principal_payment
    return total_interest
...
if st.button('Compare'):
    total_interest_fixed = calculate_total_interest_fixed(principal_fixed, fixed_rate, total_years_fixed)
    drop_months = variable_period_months
    fixed_months = total_years_variable * 12 - drop_months
    total_interest_variable = calculate_total_interest_variable(principal_variable, variable_rate, rate_drop, fixed_rate_after_variable, drop_months, fixed_months)
